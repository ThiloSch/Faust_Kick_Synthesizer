...
// Additional Controls
physmix = hslider("mix",-1,-1,1,0.1);                                                    // Classic/Physical Mix
sigType = hslider("Shape", 0, 0, 1.5, 0.01);                                            // Signal Type
frequency = hslider("/h:[0]Pitch/frequency", 50, 40, 100, 0.1);                         // Endfrequency
p_release = 0.1;
punch = hslider("Punch",0.5,0.001,1,0.001);

//OSCILLATOR SECTION//
//-----------------//
oscillator_sin(sig) = signal
with{
    swing = pitcher(40+punch*400, frequency, 0.04-punch*0.030, p_release, trigger);
    coef = 1;
...
//-----------------//
character = hslider("Character",0.5,0.001,1,0.001);
physfreq = frequency;

striker = pm.impulseExcitation(trigger) : chain;

meshed = striker : drumModel(frequency,0,1,1,2000*character) : *(en.ar(attack,10,trigger)); //
...
modesFreqs(i) = freq/(2-character*1.5)*modesFreqRatios(i);
